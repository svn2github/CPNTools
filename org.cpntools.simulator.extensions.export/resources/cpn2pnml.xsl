...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	version="1.0" xmlns:pnml="http://www.pnml.org/version-2009/grammar/pnml"
	xmlns:cpn="http://www.daimi.au.dk/CPNTools/hlpn">

	<!-- TODO - Port/Socket for In - Handle I/O ports - Arc expressions -->
	...
	<xsl:template match="color" mode="decl">
		<pnml:declaration>
			<pnml:type>
				...
				<pnml:type>
					<xsl:apply-templates select="int|bool|string|enum|product"
						mode="decl" />
				</pnml:type>
			</pnml:type>
		</pnml:declaration>
	...
	<xsl:template match="int" mode="decl">
		<pnml:int />
	</xsl:template>
	<xsl:template match="bool" mode="decl">
		<pnml:bool />
	</xsl:template>
	<xsl:template match="string" mode="decl">
		<pnml:string />
	</xsl:template>
	...
	<xsl:template match="place" priority="1">
		<pnml:place>
			...
			<pnml:graphics>
				<xsl:apply-templates select="posattr|fillattr|lineattr|ellipse|box"
					mode="graphics" />
			</pnml:graphics>
		</pnml:place>
	</xsl:template>
	...
	<xsl:template match="place[fusioninfo]" priority="2">
		<xsl:variable name="fusname">
			<xsl:value-of select="fusioninfo/@name" />
		</xsl:variable>
	...
	<xsl:template match="place" priority="1" mode="fusion">
		<xsl:variable name="fusiongroup">
			<xsl:value-of select="@id" />
		</xsl:variable>
		<pnml:globalPlace
			id="{generate-id(//fusion/fusion_elm[@idref=$fusiongroup])}">
			<xsl:apply-templates select="text" />
			<xsl:apply-templates select="initmark" />
			<xsl:apply-templates select="type" />
			<pnml:graphics>
	...
	<!-- <xsl:template match="place[port]" priority="2" mode="fusion"> <pnml:referencePlace 
		ref="{generate-id()}"> <xsl:attribute name="id"><xsl:value-of select="@id"/></xsl:attribute> 
		<pnml:graphics> <xsl:apply-templates select="posattr|fillattr|lineattr|ellipse|box" 
		mode="graphics"/> </pnml:graphics> </pnml:referencePlace> </xsl:template> -->
	...
	<xsl:template match="Aux[text]" priority="2">
		<pnml:toolspecific tool="CPN Tools" version="1.0.0">
			<cpn:aux>
				...
				<cpn:graphics>
					<xsl:apply-templates select="posattr|textattr"
						mode="cpngraphics" />
				</cpn:graphics>
			</cpn:aux>
		</pnml:toolspecific>
	...
	<xsl:template name="writePortSocket">
		...
		<xsl:if test="not($assignments = '')">
			<xsl:variable name="assignment">
				<xsl:value-of select="substring-before($assignments, ')')" />
			</xsl:variable>
			<xsl:variable name="first">
				<xsl:value-of
					select="substring-after(substring-before($assignment, ','), '(')" />
			</xsl:variable>
			<xsl:variable name="second">
				<xsl:value-of select="substring-after($assignment, ',')" />
			</xsl:variable>
			<xsl:if test="//place[@id=$first]/port">
				<pnml:importPlace>
					<xsl:attribute name="parameter"><xsl:value-of
						select="generate-id(//place[@id=$first])" /></xsl:attribute>
					<xsl:attribute name="ref"><xsl:value-of
						select="$second" /></xsl:attribute>
				</pnml:importPlace>
			</xsl:if>
			<xsl:call-template name="writePortSocket">
				<xsl:with-param name="assignments"
					select="substring-after($assignments, ')')" />
			</xsl:call-template>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="trans[subst]" priority="2">
		<pnml:transition>
			...
			<xsl:apply-templates select="text" />
			<xsl:call-template name="writePortSocket">
				<xsl:with-param name="assignments" select="subst/@portsock" />
			</xsl:call-template>
			<pnml:graphics>
				<xsl:apply-templates select="posattr|fillattr|lineattr|ellipse|box"
					mode="graphics" />
			...
			<pnml:graphics>
				...
				<pnml:font family="arial" size="12" style="normal" align="left">
					<xsl:attribute name="weight">
						<xsl:choose>
							<xsl:when
						test="translate(../textattr/@bold,$ucletters,$lcletters)='false'">normal</xsl:when>
	...
	<xsl:template match="fillattr" mode="cpngraphics"
		priority="1">
		<cpn:fill>
			<xsl:attribute name="color">
				<xsl:call-template name="tolower">
	...
	<xsl:template match="fillattr[@filled='false']" mode="cpngraphics"
		priority="2">
	...
	<xsl:template match="posattr" mode="graphics">
		...
		<xsl:variable name="wmax">
			<xsl:call-template name="maxvalue">
				<xsl:with-param name="values"
					select="/workspaceElements//ellipse/@w | /workspaceElements//box/@w" />
				<xsl:with-param name="max" select="0" />
			</xsl:call-template>
		</xsl:variable>
		<xsl:variable name="hmax">
			<xsl:call-template name="maxvalue">
				<xsl:with-param name="values"
					select="/workspaceElements//ellipse/@h | /workspaceElements//box/@h" />
				<xsl:with-param name="max" select="0" />
			</xsl:call-template>
		</xsl:variable>
		<pnml:position>
			<!--HV: Compute new coordinates (invert y, use (1 + half max width, 1 
				+ half max height) as offset) -->
			<xsl:attribute name="x"><xsl:value-of
				select="1 + $wmax div 2 + @x - $xmin" /></xsl:attribute>
			<xsl:attribute name="y"><xsl:value-of
				select="1 + $hmax div 2 + $ymax - @y" /></xsl:attribute>
		</pnml:position>
	</xsl:template>
	...
	<xsl:template match="place/fillattr|trans/fillattr" mode="graphics"
		priority="1">
		<pnml:fill>
			<xsl:attribute name="color">
				<xsl:call-template name="toRGB">
	...
	<xsl:template
		match="place/fillattr[@filled='false']|trans|fillattr[@filled='false']"
		mode="graphics" priority="2">
	...
	<xsl:template match="fusion">
		<pnml:module>
			<xsl:attribute name="id"><xsl:value-of select="@id" /></xsl:attribute>
			<xsl:variable name="representant">
				<xsl:value-of select="fusion_elm[1]/@idref" />
			</xsl:variable>
			<pnml:interface>
			...
			<xsl:apply-templates select="//place[@id=$representant]"
				mode="fusion" />
		</pnml:module>
	</xsl:template>
